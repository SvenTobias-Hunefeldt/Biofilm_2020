...
rarefaction_curve_data <- calculate_rarefaction_curves(Phyloseq_Biofilm_16S, c('Observed'), rep(c(1, 10, 100, 1:100 * 1000), each = 10))
#Summarise 
...
rarefaction_curve_data_summary_verbose <- merge(rarefaction_curve_data_summary, data.frame(sample_data(Phyloseq_Biofilm_16S))
                                                , by.x = 'Sample'
                                                , by.y = 'row.names'
                                                )

#Make plot
Rarecurve_16S = ggplot(data = rarefaction_curve_data_summary_verbose,
  mapping = aes(x = Depth,
...
```{r}
#Use function to calculate curve
rarefaction_curve_data <- calculate_rarefaction_curves(Phyloseq_Biofilm_18S, c('Observed'), rep(c(1, 10, 100, 1:100 * 1000), each = 10))
#Summarise 
summary(rarefaction_curve_data)
...
#Merge phyloseq metadata and the rarefaction summary
rarefaction_curve_data_summary_verbose <- merge(rarefaction_curve_data_summary, data.frame(sample_data(Phyloseq_Biofilm_18S))
                                                , by.x = 'Sample'
                                                , by.y = 'row.names'
                                                )

#Make plot
Rarecurve_18S = ggplot(data = rarefaction_curve_data_summary_verbose,
  mapping = aes(x = Depth,
                y = Alpha_diversity_mean,
                ymin = Alpha_diversity_mean - Alpha_diversity_sd,
                ymax = Alpha_diversity_mean + Alpha_diversity_sd,
