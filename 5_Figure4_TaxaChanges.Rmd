...
#Figure 5 A - prokaryotic community changes
```{r Set up}

#saveRDS(Summary_plot_Time, "/YOUR/PATH/HERE/Sign_16S_taxa.rds")
```
```{r Calculate means}
...
test_mean = dat_2fold_FDR_Time %>% 
  group_by(Phylum, StageofBiofilm, Substrate) %>% 
  dplyr::summarise(mean_val = mean(Abundance), sd = sd(Abundance))
print(test_mean, n = 56)
#Substrate differences
test_mean$mean_val[35] - ((test_mean$mean_val[33]+test_mean$mean_val[34]+test_mean$mean_val[36])/3) #Proteobacteria
((test_mean$mean_val[9]+test_mean$mean_val[10]+test_mean$mean_val[12])/3) - test_mean$mean_val[11]  #Cyanobacteria
...
test_mean = Summary_Time_Phylum_noUnknown %>% 
...
test_mean$sd[15]
```
#Figure 5 B - eukaryotic community changes
```{r Set up}
...
Summary_Time$Substrate = gsub("Acryl", "Plastic", Summary_Time$Substrate)
Summary_Time$Substrate = gsub("Pine", "Wood", Summary_Time$Substrate)
...
Summary_Time_Phylum_noUnknown$Mesh_status = gsub("Exposed", "Native", Summary_Time_Phylum_noUnknown$Mesh_status)
#Summary_Time_Phylum_noUnknown$Mesh_status = gsub("Enclosed", "Exclusive", Summary_Time_Phylum_noUnknown$Mesh_status)
...
#saveRDS(Summary_plot_Time, "/YOUR/PATH/HERE/Sign_18S_taxa.rds")
...
test_mean = dat_2fold_FDR_Time %>% 
  group_by(Phylum, StageofBiofilm, Substrate) %>% 
  dplyr::summarise(mean_val = mean(Abundance), sd = sd(Abundance))
print(test_mean, n = 64)
test_mean$mean_val[19] - ((test_mean$mean_val[17]+test_mean$mean_val[18]+test_mean$mean_val[20])/3) #Euglenozoa
((test_mean$mean_val[49]+test_mean$mean_val[50]+test_mean$mean_val[52])/3) - test_mean$mean_val[51] #Euglenozoa
...
dat_2fold_FDR_Time$Organism_type <- factor(dat_2fold_FDR_Time$Organism_type,
                         levels=c("Primary_Producers", "Heterotrophs", "Mixotrophs", "Unknown"))
dat_2fold_FDR_Time$Organism_type[grep("Cyanobacteria", dat_2fold_FDR_Time$Phylum)] = as.factor("Primary_Producers")
dat_2fold_FDR_Time$Organism_type[grep("Epsilonbacteraeota", dat_2fold_FDR_Time$Phylum)] = as.factor("Primary_Producers")
...
#Save plot
tiff("/YOUR/PATH/HERE/Figures_Tables/Figure4ms_SignPhylaChanges.tiff", width = 16, height = 13, units = "in", res = 300)
Figure4ms
dev.off()
...
dat_2fold_FDR_Time <- tax_glom(Twofold_FDR_Time, taxrank = 'Genus') %>%#Merge the species at the Genus level
  transform_sample_counts(function(x) {x/sum(x)} ) %>% #Extract abundance data from phyloseq object
  psmelt() #Melt it into a dataframe

dat_2fold_FDR_Time <- dat_2fold_FDR_Time[order(dat_2fold_FDR_Time$Genus),] #Order them at the Genus level

dat_2fold_FDR_Time$Genus <- as.character(dat_2fold_FDR_Time$Genus)
  # group dataframe by Genus, calculate relative abundance

#Remove na values
dat_2fold_FDR_Time$Abundance[grep(NaN, dat_2fold_FDR_Time$Abundance)] = 0

#Calculate mean abundance
medians_Time <- ddply(dat_2fold_FDR_Time, ~Genus, function(x) c(median=mean(x$Abundance)))
  # find Phyla whose rel. abund. is less than 1%
remainder_Time <- medians_Time[medians_Time$median <= 0.01,]$Genus
  # change their name to "Remainder"
dat_2fold_FDR_Time[dat_2fold_FDR_Time$Genus %in% remainder_Time,]$Genus <- 'Rare Taxa'
...
Summary_Time<-dplyr::arrange(Summary_Time,Genus, Abundance)
#Get rid of excess phyla that are not shown
Summary_Time$Genus <- factor(Summary_Time$Genus,
                         levels=(unique(Summary_Time$Genus)))
#Identify what phyla we have
unique(Summary_Time$Genus)
...
#For back up
#write.csv(Summary_Time, file = "Summary_Time_Genus_16S.csv")
...
Summary_Time_Genus_noUnknown = subset(Summary_Time)
#Convert and rename for consistency across figures
Summary_Time_Genus_noUnknown$Mesh_status = as.character(Summary_Time_Genus_noUnknown$Mesh_status)
Summary_Time_Genus_noUnknown$Mesh_status = gsub("Exposed", "Native", Summary_Time_Genus_noUnknown$Mesh_status)
#Summary_Time_Genus_noUnknown$Mesh_status = gsub("Enclosed", "Exclusive", Summary_Time_Genus_noUnknown$Mesh_status)
Summary_Time_Genus_noUnknown$Sample_time = as.factor(as.character(Summary_Time_Genus_noUnknown$Sample_time))
Summary_Time_Genus_noUnknown$Sample_time = factor(Summary_Time_Genus_noUnknown$Sample_time,
                         levels=c("7",
                                  "14",
                                  "19",
                                  "28",
                                  ...
                                  "56"))
library(viridis)
Summary_plot_Time_16S_genera = ggplot(Summary_Time_Genus_noUnknown, 
                        aes(x=Sample_time, 
                            y= fct_reorder(Genus, Abundance, .fun = mean, .desc = F), 
                            fill = Abundance*100,
                            group = Genus))+ 
  facet_grid(Substrate ~ Mesh_status)+
  geom_tile(stat = "identity", lwd = 1) + 
  scale_fill_continuous(type = "viridis")+
  ggtitle("Prokaryotes")+
  
  xlab("Time (days)")+
  ylab("Genus")+
  labs(fill = "Mean abundance (%)")+
  theme_bw()+
  theme(legend.position = "right", 
        strip.background = element_blank())+
  theme(plot.title = element_text(hjust = 0.5))+
...
Summary_plot_Time_16S_genera

tiff("/YOUR/PATH/HERE/Figures_Tables/SupplementaryFigure8_ProkaryoticGeneraHeatmap.tiff", width = 16, height = 24, units = "in", res = 300)
Summary_plot_Time_16S_genera
dev.off()
...
#For back up
#write.csv(Summary_Time, file = "Summary_Time_Genus_18S.csv")
...
Summary_plot_Time_18S_genera = ggplot(Summary_Time_Genus_noUnknown, 
                        aes(x=Sample_time, 
                            y= fct_reorder(Genus, Abundance, .fun = mean, .desc = F), 
                            fill = Abundance*100,
                            group = Genus))+ 
...
Summary_plot_Time_18S_genera

tiff("/YOUR/PATH/HERE/Figures_Tables/SupplementaryFigure9_EukaryoticGeneraHeatmap.tiff", units = "in", res = 300, width = 16, height = 24)
Summary_plot_Time_18S_genera
dev.off()
#saveRDS(Summary_plot_Time, "/YOUR/PATH/HERE/Sign_18S_taxa.rds")
```
#Terrestial taxa
```{r}
Wood_phyloseq = subset_samples(Phyloseq_Biofilm_16S, Substrate == "Pine")
sum(taxa_sums(Wood_phyloseq) < 1) #How many below 1?
Wood_phyloseq = prune_taxa(taxa_sums(Wood_phyloseq) > 1, Wood_phyloseq)
Wood_ASV.df <- tax_glom(Wood_phyloseq, taxrank = 'OTU') %>%#Merge the species at the Genus level
  transform_sample_counts(function(x) {x/sum(x)} ) %>% #Extract abundance data from phyloseq object
  psmelt() #Melt it into a dataframe

AllButWood_phyloseq = subset_samples(Phyloseq_Biofilm_16S, Substrate!="Pine")
sum(taxa_sums(AllButWood_phyloseq) < 1) #How many below 1?
AllButWood_phyloseq = prune_taxa(taxa_sums(AllButWood_phyloseq) > 1, AllButWood_phyloseq)
NoWood_ASV.df = tax_glom(AllButWood_phyloseq, taxrank = 'OTU') %>%#Merge the species at the Genus level
  transform_sample_counts(function(x) {x/sum(x)} ) %>% #Extract abundance data from phyloseq object
  psmelt() #Melt it into a dataframe

'%!in%' <- function(x,y)!('%in%'(x,y))

NoWoodASVList = unique(NoWood_ASV.df$taxa_OTU)
Wood_specific_ASVs = unique(Wood_ASV.df$taxa_OTU)

Wood_specific_ASVs = subset(Wood_ASV.df, taxa_OTU %!in% NoWood_ASV.df$taxa_OTU)

Lookuplist = as.data.frame(unique(Wood_specific_ASVs$Genus))
```
#Significant differences between enclosure conditions
##16S
```{r}
#Restore a pristine copy from back up
Phyloseq_Biofilm_16S = Phyloseq_Biofilm_16S_v0
...
#Convert phyloseq object to edgeR object using cutsom function
dge_EdgeR_obj_Time = phyloseq_to_edgeR(Phyloseq_Biofilm_16S_noA, group = "Mesh_status")
...
keepTaxa_FDR_Time <- sigtab_2fold_EdgeR_Time$Genus #Extract the significant genera
Twofold_FDR_Time <- subset_taxa(Phyloseq_Biofilm_16S_noA, Genus %in% keepTaxa_FDR_Time) #Subset the taxa by the OTUs that were shown to change significantly
dat_2fold_FDR_Time <- tax_glom(Twofold_FDR_Time, taxrank = 'Class') %>%#Merge the species at the Class level
  transform_sample_counts(function(x) {x/sum(x)} ) %>% #Extract abundance data from phyloseq object
  psmelt() #Melt it into a dataframe

dat_2fold_FDR_Time <- dat_2fold_FDR_Time[order(dat_2fold_FDR_Time$Class),] #Order them at the Class level

dat_2fold_FDR_Time$Class <- as.character(dat_2fold_FDR_Time$Class)
  # group dataframe by Class, calculate relative abundance

#Remove na values
dat_2fold_FDR_Time$Abundance[grep(NaN, dat_2fold_FDR_Time$Abundance)] = 0

#Calculate mean abundance
medians_Time <- ddply(dat_2fold_FDR_Time, ~Class, function(x) c(median=mean(x$Abundance)))
  # find Phyla whose rel. abund. is less than 1%
remainder_Time <- medians_Time[medians_Time$median <= 0.01,]$Class
  # change their name to "Remainder"
dat_2fold_FDR_Time[dat_2fold_FDR_Time$Class %in% remainder_Time,]$Class <- 'Rare Taxa'
...
#Arrange for easier viewing
Summary_Time<-dplyr::arrange(Summary_Time,Class, Abundance)
#Get rid of excess phyla that are not shown
Summary_Time$Class <- factor(Summary_Time$Class,
                         levels=(unique(Summary_Time$Class)))
#Identify what phyla we have
unique(Summary_Time$Class)
...
#For back up
#write.csv(Summary_Time, file = "Summary_Time_Class_16S.csv")
...
```{r Refine and make plot}
#Remove unknown ecological functions
Summary_Time_Class_noUnknown = subset(Summary_Time)
#Convert and rename for consistency across figures
Summary_Time_Class_noUnknown$Mesh_status = as.character(Summary_Time_Class_noUnknown$Mesh_status)
Summary_Time_Class_noUnknown$Mesh_status = gsub("Exposed", "Native", Summary_Time_Class_noUnknown$Mesh_status)
Summary_Time_Class_noUnknown$Mesh_status = gsub("Enclosed", "Exclusive", Summary_Time_Class_noUnknown$Mesh_status)
Summary_Time_Class_noUnknown$Sample_time = as.factor(as.character(Summary_Time_Class_noUnknown$Sample_time))
Summary_Time_Class_noUnknown$Sample_time = factor(Summary_Time_Class_noUnknown$Sample_time,
                         levels=c("7",
                                  "14",
                                  "19",
                                  "28",
                                  ...
                                  "56"))
library(viridis)
Summary_plot_Time_16S_genera = ggplot(Summary_Time_Class_noUnknown, 
                        aes(x=Sample_time, 
                            y= fct_reorder(Class, Abundance, .fun = mean, .desc = F), 
                            fill = Abundance*100,
                            group = Class))+ 
  facet_grid(Substrate ~ Mesh_status)+
  geom_tile(stat = "identity", lwd = 1) + 
...
#pdf("/YOUR/PATH/HERE/ProkaryoticGeneraHeatmap.pdf", width = 16, height = 24)
#Summary_plot_Time_16S_genera
#dev.off()
#saveRDS(Summary_plot_Time, "/YOUR/PATH/HERE/Sign_16S_taxa.rds")
```
...
#Convert phyloseq object to edgeR object using cutsom function
dge_EdgeR_obj_Time = phyloseq_to_edgeR(Phyloseq_Biofilm_18S_noA, group = "Mesh_status")
...
#For back up
#write.csv(Summary_Time, file = "Summary_Time_Class_18S.csv")
...
Summary_Time_Class_noUnknown$Sample_time = factor(Summary_Time_Class_noUnknown$Sample_time,
                         levels=c("7",
                                  ...
                                  "56"))
library(viridis)
Summary_plot_Time_18S_genera = ggplot(Summary_Time_Class_noUnknown, 
                        aes(x=Sample_time, 
                            y= fct_reorder(Class, Abundance, .fun = mean, .desc = F), 
                            fill = Abundance*100,
                            group = Class))+ 
...
unique(Summary_Time_Class_noUnknown$Class)
