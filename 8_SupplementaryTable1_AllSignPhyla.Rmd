...
#Summarise for table
Summary_Time_Phylum_Prok <- summarySE(dat_2fold_FDR_Time, measurevar="Abundance", groupvars=c("Kingdom", "Phylum", "Mesh_status", "Sample_time", "StageofBiofilm", "Substrate"))
Summary_Time_Phylum_Prok


#Arrange and reduce file size for easier visualisation
Summary_Time_Phylum_Prok<-dplyr::arrange(Summary_Time_Phylum_Prok, Phylum, Phylum, Abundance)
Summary_Time_Phylum_Prok$Phylum <- factor(Summary_Time_Phylum_Prok$Phylum,
                         levels=(unique(Summary_Time_Phylum_Prok$Phylum)))

#write.csv(Summary_Time_Phylum, file = "~/Desktop/Biofilm_project/Analysis/Clean_Thesis_Code/Tables/Summary_Time_Phylum_16S.csv")

#Check for weird names and make Rare taxa name better.
levels(Summary_Time_Phylum_Prok$Phylum)
```
```{r Eukaryotes}
...
#Summarise for table
Summary_Time_Phylum_Euk <- summarySE(dat_2fold_FDR_Time, measurevar="Abundance", groupvars=c("Kingdom", "Phylum", "Mesh_status", "Sample_time", "StageofBiofilm", "Substrate"))
Summary_Time_Phylum_Euk

#Reorder and arrange for easier viewing
Summary_Time_Phylum_Euk<-dplyr::arrange(Summary_Time_Phylum_Euk, Phylum, Phylum, Abundance)
Summary_Time_Phylum_Euk$Phylum <- factor(Summary_Time_Phylum_Euk$Phylum,
                         levels=(unique(Summary_Time_Phylum_Euk$Phylum)))
#write.csv(Summary_Time_Phylum_Euk, file = "~/Desktop/Biofilm_project/Analysis/Clean_Thesis_Code/Tables/Summary_Time_Phylum_18S.csv")

#Check for weird names and make Rare taxa name better.
levels(Summary_Time_Phylum_Euk$Phylum)
levels(Summary_Time_Phylum_Euk$Phylum)
```
```{r Combine into one csv file}
#Ensure that dimensions match
dim(Summary_Time_Phylum_Prok)
dim(Summary_Time_Phylum_Euk)
#Combine rows
Summary_Time_Phylum_all = rbind(Summary_Time_Phylum_Prok, Summary_Time_Phylum_Euk)

#Rename substrates
Summary_Time_Phylum_all$Substrate = gsub("Acryl", "Plastic", Summary_Time_Phylum_all$Substrate)
Summary_Time_Phylum_all$Substrate = gsub("Pine", "Wood", Summary_Time_Phylum_all$Substrate)

#Rename enclosure conditions
Summary_Time_Phylum_all$Mesh_status = gsub("Exposed", "Native", Summary_Time_Phylum_all$Mesh_status)
Summary_Time_Phylum_all$Mesh_status = gsub("Enclosed", "Exclusive", Summary_Time_Phylum_all$Mesh_status)

#Extract table
write.csv(Summary_Time_Phylum_all, file = "~/Desktop/Biofilm_project/Analysis/Clean_ms_Code/Figures_Table/SupplementaryTable1_AgeCorrelatePhyla.csv")
